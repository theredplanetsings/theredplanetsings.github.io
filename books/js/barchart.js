class Barchart {
  ...
  renderVis(counts) {
    ...
    bars.join(
      ...
      // HOVER & FOCUS (tooltip + speech)
      .on('mouseover',(e,d) => { showTooltip(e,d); speak(e.currentTarget.getAttribute('aria-label')); })
      .on('focus',    (e)   => { 
      ...
      .on('keydown',  (e,d) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
          vis.config.dispatcher.call('toggleGenre', null, d.genre);
        }
      })
    ...
    // **after** the join, re-stroke the bars that are in genreFilter**
